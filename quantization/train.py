...
from utils import init_logging, segDataset, unet, bootstrapped_cross_entropy2d, DiceLoss
...
train_dataloader = DataLoader(train_set, batch_size=BATCH_SIZE, num_workers=4, shuffle=True,drop_last=True,pin_memory = False)	
val_dataloader = DataLoader(val_set, batch_size=BATCH_SIZE, num_workers=4, shuffle=False,drop_last=True,pin_memory = False)
...
#celoss = bootstrapped_cross_entropy2d().to(device)
...
if optimType ==	"Adam":	
    ...
    optimizer = torch.optim.SGD(params=model.parameters(), lr=lr0, momentum=0.9, dampening=0.5, weight_decay=wd0, nesterov=False)
else:
...
def eval(model, epoch):	
    ...
    logging.info("eval E:{:02}/{:03} all:{:.6f} ce:{:.6f} dice:{:.6f}".format(epoch, NUM_EPOCH, _all_loss, _ce_loss, _dice_loss))	
    logging.info("-"*56)	
...
if __name__ == "__main__":
    for epoch in range(Start_epoch, NUM_EPOCH):	
        ...
        torch.save(model.state_dict(), "./checkpoints/{}/epoch_{:02}_loss_{:.6f}_ce_{:.6f}_dice_{:.6f}.pth".format(experment_name,epoch, all_loss, ce_loss, dice_loss),_use_new_zipfile_serialization=False)
