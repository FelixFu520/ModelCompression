...
def init_logging(log_file=None, file_mode='w', overwrite_flag=True, log_level=logging.DEBUG):
    ...
    if not os.path.exists(log_file):
        ...
        if overwrite_flag:
            print('The file {} is existed. And it is to be handled according to the arg [file_mode](the default is \'w\').'.format(log_file)) 
            fileHandler = logging.FileHandler(filename=log_file, mode=file_mode)
            fileHandler.setFormatter(formatter)
            logging.getLogger().addHandler(fileHandler)
        else:
            print('The file {} is existed. The [overwrite_flag] is False, please change the [log_file].'.format(log_file)) 
            quit()
